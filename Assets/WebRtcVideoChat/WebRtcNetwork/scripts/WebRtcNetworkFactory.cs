...
namespace Byn.Net
{
    ...
    public class WebRtcNetworkFactory : UnitySingleton<WebRtcNetworkFactory>
    {
...
#elif UNITY_IOS
			Debug.LogWarning("Trying to initialize native webrtc for IOS.  Note that this OS isn't fully supported yet!");
#else
        ...
        public static void TryStaticInitialize()
        {
            ...
            if (sStaticInitSuccessful == false)
			{
				Debug.LogError("Platform not supported. Returning without initializing");
				return;
			}
...
//keep in editor to not miss compiler errors. only add to android builds
#if UNITY_ANDROID || UNITY_EDITOR
